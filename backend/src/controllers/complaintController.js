...
export const updateComplaintStatus = async (req, res) => {
  try {
    ...
    console.log("Status update request:", { complaintId, status, userRole: req.user.role, userId: req.user.id });
    ...
    // Allow admin to update any complaint, staff can only update assigned complaints
    if (req.user.role === "staff" && (!complaint.assigned_to || String(complaint.assigned_to) !== req.user.id)) {
      return res.status(403).json({ success: false, message: "Not your complaint" });
    }
